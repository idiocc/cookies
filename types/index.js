 ...
 * @prop {!_goa.Keygrip} [keys] The keys object constructed from passed keys (private, will be installed
      from options).
 * @prop {boolean} [secure] Explicitly specifies if the connection is secure (private, will be installed
      from options).
 * @prop {function(string, { signed: boolean }): (string|undefined)} get This extracts the cookie with the given name from the Cookie header in
 ...
 * @prop {function(string, ?string=, _goa.CookieAttributes=)} set This sets the given cookie in the response and returns the current context
 ...
 * @prop {boolean} [secure] Explicitly specifies if the connection is secure, rather than this module
      examining request.
 */
/**
 ...
 * @prop {!Date} [expires] Indicates the cookie's expiration date (expires at the end of session by
      default).
 * @prop {string} [path="/"] Indicates the path of the cookie. Default `/`.
 * @prop {string} [domain] Indicates the domain of the cookie.
 * @prop {boolean} [secure] Indicates whether the cookie is only to be sent over HTTPS (false by default
      for HTTP, true by default for HTTPS).
 * @prop {number} [httpOnly=true] Indicates whether the cookie is only to be sent over HTTP(S), and not made
      available to client JavaScript. Default `true`.
 * @prop {boolean|string} [sameSite=false] Indicates whether the cookie is a "same site" cookie. This can be set to
      `'strict'`, `'lax'`, or `true` (which maps to `'strict'`). Default `false`.
 * @prop {boolean} [signed=false] Indicating whether the cookie is to be signed. If this is true, another cookie
 ...
 * @prop {boolean} [overwrite=false] Indicates whether to overwrite previously set cookies of the same name. If
 ...
 * @prop {function(?): string} sign This creates a SHA1 HMAC based on the _first_ key in the keylist, and outputs
      it as a 27-byte url-safe base64 digest (base64 without padding, replacing `+`
      with `-` and `/` with `_`).
 * @prop {function(?, string): number} index This loops through all of the keys currently in the keylist until the
 ...
 * @prop {function(?, string): boolean} verify This uses `index` to return true if the digest matches any existing keys,
      and false otherwise.
 */
